...
// Every other iPhone/iPad check stays on WebKit in the other e2e specs.
...
test('the app shell loads fully offline from the service worker cache', async ({
  ...
  annotate(
    test.info(),
    CATEGORY.OFFLINE,
    'After the service worker has cached the app shell, go offline (Airplane mode) and reload — the app must still render a fresh document from cache.',
  );
